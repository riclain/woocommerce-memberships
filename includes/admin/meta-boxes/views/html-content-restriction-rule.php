		...
		<?php if ( $rule->get_membership_plan_id() == $post->ID ) : ?>
			...
			<td class="content-restriction-content-type content-type-column">
				<p class="form-field">
					...
					<select name="_content_restriction_rules[<?php echo esc_attr( $index ); ?>][content_type_key]"
					        id="_content_restriction_rules_<?php echo esc_attr( $index ); ?>_content_type_key"
					        class="js-content-type" <?php if ( ! $rule->current_user_can_edit() ) : ?>disabled<?php endif; ?>>
		...
		<?php else : ?>
			...
			<td class="content-restriction-membership-plan membership-plan-column">
				<p class="form-field">
					...
					<select name="_content_restriction_rules[<?php echo esc_attr( $index ); ?>][membership_plan_id]"
			...
			<p class="form-field">
				...
				<span class="rule-control-group rule-control-group-access-schedule-specific js-hide-if-access-schedule-immediate <?php if ( 'immediate' == $rule->get_access_schedule() ) : ?>hide<?php endif;?>">
					...
					<input type="number"
					       ...
					       value="<?php echo esc_html( $rule->get_access_schedule_amount() ); ?>"
					       min="0"
					       <?php if ( ! $rule->current_user_can_edit() || ! $rule->current_context_allows_editing() ) : ?>disabled<?php endif; ?> />
	...
	<?php if ( ! $rule->current_user_can_edit() || ! $rule->current_context_allows_editing() ) : ?>
		...
		<tr class="disabled-notice">
			...
			<td colspan="<?php echo ( 'wc_membership_plan' == $post->post_type ) ? 4 : 3; ?>">
				...
				<?php else : ?>
					<span class="description"><?php printf( esc_html__( 'This rule cannot be edited here because it applies to multiple content objects. You can %sedit this rule on the membership plan screen%s.', 'woocommerce-memberships' ), '<a href="' . esc_url( get_edit_post_link( $rule->get_membership_plan_id() ) ) . '">', '</a>' ); ?></span>
				<?php endif; ?>
